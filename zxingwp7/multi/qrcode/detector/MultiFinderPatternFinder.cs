...
namespace zxingwp7.multi.qrcode.detector
{
    ...
    internal sealed class MultiFinderPatternFinder : FinderPatternFinder
    {
        ...
        {
            List<ResultPoint> possibleCenters = PossibleCenters;
                ...
                return new[]
                           {
                               new[]
                                   {
                                       (FinderPattern) possibleCenters[0], (FinderPattern) possibleCenters[1],
			...
			*/
            ...
            var results = new List<FinderPattern[]>(10); // holder for the results

            for (int i1 = 0; i1 < (size - 2); i1++)
            {
                var p1 = (FinderPattern) possibleCenters[i1];
                if (p1 == null)
                {
                    continue;
                }
                ...
                {
                    var p2 = (FinderPattern) possibleCenters[i2];
                    if (p2 == null)
                    {
                        continue;
                    }
                    ...
                    {
                        var p3 = (FinderPattern) possibleCenters[i3];
                        if (p3 == null)
                        {
                            continue;
                        }
                        ...
                        var test = new[] {p1, p2, p3};
                        ResultPoint.orderBestPatterns(test);

                        // Calculate the distances: a = topleft-bottomleft, b=topleft-topright, c = diagonal
                        var info = new FinderPatternInfo(test);
                        ...
                        var dCpy = (float) Math.Sqrt(dA*dA + dB*dB);
            ...
            if (!(results.Count == 0))
            {
                var resultArray = new FinderPattern[results.Count][];
        ...
        public FinderPatternInfo[] findMulti(Dictionary<DecodeHintType, Object> hints)
        {
            ...
            var iSkip = (int) (maxI/(MAX_MODULES*4.0f)*3);
            ...
            var result = new List<FinderPatternInfo>(10);
            ...
            {
                var resultArray = new FinderPatternInfo[result.Count];
        ...
        #region Nested type: ModuleSizeComparator

        /// <summary> A comparator that orders FinderPatterns by their estimated module size.</summary>
        private class ModuleSizeComparator : Comparer<ResultPoint>
        {
            public override int Compare(ResultPoint x, ResultPoint y)
            {
                float value_Renamed = ((FinderPattern)y).EstimatedModuleSize -
                                      ((FinderPattern)x).EstimatedModuleSize;
