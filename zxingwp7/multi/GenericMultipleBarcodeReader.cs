...
namespace zxingwp7.multi
{
    ...
    public sealed class GenericMultipleBarcodeReader : MultipleBarcodeReader
    {
        ...
        private void doDecodeMultiple(BinaryBitmap image, Dictionary<DecodeHintType, Object> hints, List<Result> results,
                                      int xOffset, int yOffset)
        {
            ...
            var alreadyFound = results.Any(existingResult => existingResult.Text.Equals(result.Text));
            ...
            if (maxY < height - MIN_DIMENSION_TO_RECUR)
            {
                ...
                doDecodeMultiple(image.crop(0, (int) maxY, width, height - (int) maxY), hints, results, xOffset,
                                 yOffset + (int) maxY);
            }
        }
