...
namespace zxingwp7.qrcode.decoder
{
    ...
    internal sealed class DecodedBitStreamParser
    {
        ...
        private static readonly char[] ALPHANUMERIC_CHARS = new[]
                                                                {
        ...
        private static String guessEncoding(sbyte[] bytes)
        {
            ...
            if (bytes.Length > 3 && bytes[0] == (sbyte) SupportClass.Identity(0xEF) &&
                bytes[1] == (sbyte) SupportClass.Identity(0xBB) && bytes[2] == (sbyte) SupportClass.Identity(0xBF))
            {
                return UTF8;
            }
            ...
            for (int i = 0; i < length && (canBeISO88591 || canBeShiftJIS); i++)
            {
                ...
                if ((value_Renamed == 0xC2 || value_Renamed == 0xC3) && i < length - 1)
                {
                    ...
                    if (nextValue <= 0xBF &&
                        ((value_Renamed == 0xC2 && nextValue >= 0xA0) || (value_Renamed == 0xC3 && nextValue >= 0x80)))
                    {
                ...
                if (!lastWasPossibleDoubleByteStart &&
                    ((value_Renamed >= 0xF0 && value_Renamed <= 0xFF) || value_Renamed == 0x80 || value_Renamed == 0xA0))
                {
                    canBeShiftJIS = false;
                }
                if (((value_Renamed >= 0x81 && value_Renamed <= 0x9F) ||
                     (value_Renamed >= 0xE0 && value_Renamed <= 0xEF)))
                {
