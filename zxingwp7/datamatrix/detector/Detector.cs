...
using zxingwp7.common.detector;

namespace zxingwp7.datamatrix.detector
{
    ...
    public sealed class Detector
    {
        ...
        private static readonly Int32[] INTEGERS = new[] {0, 1, 2, 3, 4};
        // No, can't use valueOf()


        private readonly BitMatrix image;

        private readonly MonochromeRectangleDetector rectangleDetector;
        ...
        public DetectorResult detect()
        {
            ...
            var transitions = new List<ResultPointsAndTransitions>(4)
                                  {
                                      transitionsBetween(pointA, pointB),
                                      transitionsBetween(pointA, pointC),
                                      transitionsBetween(pointB, pointD),
                                      transitionsBetween(pointC, pointD)
                                  };
            ...
            var pointCount = new Dictionary<ResultPoint, Int32>();
            ...
            Dictionary<ResultPoint, int>.KeyCollection.Enumerator points = pointCount.Keys.GetEnumerator();

            while (points.MoveNext())
            {

                ResultPoint point = points.Current;
                Int32 value_Renamed = pointCount[point];
                if (value_Renamed == 2)
                {
            ...
            var corners = new[] {maybeTopLeft, bottomLeft, maybeBottomRight};
            ...
            int dimension = Math.Min(transitionsBetween(topLeft, topRight).Transitions,
                                     transitionsBetween(bottomRight, topRight).Transitions);
            if ((dimension & 0x01) == 1)
            {
            ...
            return new DetectorResult(bits, new[] {pointA, pointB, pointC, pointD});
        }

        /// <summary> Increments the Integer associated with a key by one.</summary>
        private static void increment(Dictionary<ResultPoint, Int32> table, ResultPoint key)
        {
            //Int32 value_Renamed = (Int32) table[key];
                                            ...
                                            ResultPoint bottomRight, int dimension)
        {
            ...
            return sampler.sampleGrid(image, dimension, 0.0f, 0.0f, dimension, 0.0f, dimension, dimension, 0.0f,
                                      dimension, topLeft.X, topLeft.Y, topRightX, topRightY, bottomRight.X,
        ...
        #region Nested type: ResultPointsAndTransitions
        ...
        #region Nested type: ResultPointsAndTransitionsComparator

        /// <summary> Orders ResultPointsAndTransitions by number of transitions, ascending.</summary>
        private class ResultPointsAndTransitionsComparator : Comparer<ResultPointsAndTransitions>
        {
            public override int Compare(ResultPointsAndTransitions x, ResultPointsAndTransitions y)
            {
